...
?>
<h1 class="page-title"><span class="text-danger"><?=Yii::t('app', 'Предпросмотр товара:')?></span> <?=$model->name?></h1>
<p><a href="<?=Url::toRoute(['item/view', 'url' => $model->url])?>"><?=Yii::t('app', 'Перейти на полный товар, перед этим - очистите кеш в админке!')?></a></p>
<input type="hidden" name="currentElement" value="<?=$elementItem->id?>" id="currentElementId">
<div class="product-img">
    <img src="<?=$model->getImageFromItem($currentWatermark, $elementItem, true)?>" alt="<?=Html::encode($model->name)?>" id="fullImage">
    <p></p>
